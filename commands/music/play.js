...
module.exports = class playCommand extends Command {
    constructor(client) {
        super(client, {
            ...
            memberName: "play",
            description:
                "play a youtube video in a voice channel with this command",
            args: [
                {
                    key: "url",
    ...
    async songPlay(message) {
        ...
        message.channel.send(`Removed ${musicObject["title"]} from queue.`);
        ...
        this.client.music[message.guild.id].dispatcher =
            await this.client.music[message.guild.id].connection.play(
                ytdl(musicObject["url"], { quality: "highestaudio" })
            );
        this.client.music[message.guild.id].dispatcher.on(
            "finish",
            async () => {
                if (this.client.music[message.guild.id].queue.length == 0) {
                    await this.client.music[
                        message.guild.id
                    ].dispatcher.destroy();
    ...
    async run(message, { url }) {
        ...
        fetch(baseURL + url)
            ...
            .then(async (body) => {
                ...
                message.channel.send(
                    `Added ${body["title"]} to the song queue. `
                );
                ...
                await this.client.music[message.guild.id].queue.push(
                    songRequestObject
                );
                this.songPlay(message);
                return;
