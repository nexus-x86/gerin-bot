...
module.exports = class suggestCommand extends Command {
    ...
    async run(message, {suggestion}) {
        ...
        if (configEnmap.get("suggestions_"+message.guild.id) == undefined) {
            configEnmap.set("suggestions_"+message.guild.id, []);
        }
        const count = configEnmap.get("suggestions_"+message.guild.id).length + 1
        ...
        sentEmbed.react("👎");
        return;
        //return configEnmap.set("suggestions_"+message.guild.id, [count, configEnmap.get("suggestions_"+message.guild.id).push(sentEmbed.id)]);
    }
};
