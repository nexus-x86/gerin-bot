...
module.exports = class setLanguageCommand extends Command {
	...
	async run(message, {languageArg}) {
		...
		if (languageCode == "") {
			return message.channel.send("You have provided an invalid language code, or a language that we do not support. Go to https://github.com/nexus-x86/gerin-bot/wiki/Language-Codes to see all valid language codes.")
		}
		configEnmap.set("language_"+message.guild.id, languageCode);

		return message.channel.send("The language configuration has been set to " + language);
	}
};